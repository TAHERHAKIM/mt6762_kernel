...
static DEFINE_SPINLOCK(mt6762_clk_lock);
...
#define MT6762_PLL_FMAX		(3800UL * MHZ)
#define MT6762_PLL_FMIN		(1500UL * MHZ)

#define CON0_MT6762_RST_BAR	BIT(23)
...
#define PLL_B(_id, _name, _reg, _pwr_reg, _en_mask, _flags, _pcwbits,	\
		...
		.en_mask = _en_mask,					\
		.flags = _flags,					\
		.rst_bar_mask = CON0_MT6762_RST_BAR,			\
		.fmax = MT6762_PLL_FMAX,				\
		.fmin = MT6762_PLL_FMIN,				\
		.pcwbits = _pcwbits,					\
		.pcwibits = _pcwibits,					\
		.pd_reg = _pd_reg,					\
		.pd_shift = _pd_shift,					\
...
static const struct of_device_id of_match_clk_mt6762[] = {
	{
		.compatible = "mediatek,mt6762-apmixedsys",
		.data = clk_mt6762_apmixed_probe,
	}, {
		.compatible = "mediatek,mt6762-topckgen",
		.data = clk_mt6762_top_probe,
	}, {
		.compatible = "mediatek,mt6762-infracfg",
		.data = clk_mt6762_ifr_probe,
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, of_match_clk_mt6762);

static int clk_mt6762_probe(struct platform_device *pdev)
{
...
static struct platform_driver clk_mt6762_drv = {
	.probe = clk_mt6762_probe,
	.driver = {
		.name = "clk-mt6762",
		.of_match_table = of_match_clk_mt6762,
	},
};

static int __init clk_mt6762_init(void)
{
	return platform_driver_register(&clk_mt6762_drv);
}

arch_initcall(clk_mt6762_init);
MODULE_LICENSE("GPL");
